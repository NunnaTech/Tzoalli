  ...
  visitService.GetAllCompletedVisits(token)
    ...
    .then((response) => {
      ...
      if (response.data.data.length === 0) {
        ...
        <h1 class="font-bold text-2xl drop-shadow-sm  text-center text-gray-400">No cuentas con visitas completadas</h1>
        `
      } else {
          ...
          if (item.order === null || item.order === undefined) {
            ...
            <div class="card-body">
              ...
              <div class="card-actions justify-between h-8">
                <div class="badge mt-5 border-none bg-[#25AC5B] text-gray-100 py-3" style="background-color: #25ac5b; color: white">Completado</div>
                <input value="${item.id}" hidden/>
                <a for="my-modal-${i}" href="./observations.html?${item.id}"
                  class="btn btn-outline btn-success h-4 btn modal-button">Observaciones</a>
              </div>
            </div>
          </div>`
          } else {
